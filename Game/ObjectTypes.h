...
inline const char* TagToString(TagType tag) {
	...
	case TagType::IceCanon: return "IceCanon";
...
inline TagType StringToTag(const std::string& s) {
	...
	if (s == "IceCanon") return TagType::IceCanon;
...
struct CameraTargetComponent : public Component {
	float distance = 10.0f; // ★変更: 15.0f→10.0f 準備フェーズのカメラ距離を統一するため
	float height = 3.0f;    // ★変更: 4.5f→3.0f 高さも統一
