...
void MirrorShatterScript::GenerateShards(GameScene* scene, int count) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (colorType == 0)      mrc.color = { (0.8f + hueShift) * intensity, 0.7f * intensity, 0.2f * intensity, 1.0f }; // Brass (真鍮)
        else if (colorType == 1) mrc.color = { (0.7f + hueShift) * intensity, 0.4f * intensity, 0.1f * intensity, 1.0f }; // Copper (銅)
        else if (colorType == 2) mrc.color = { 2.5f * intensity, (0.5f + hueShift) * intensity, 0.1f * intensity, 1.0f }; // Red-hot Iron (赤熱)
        else                     mrc.color = { 1.5f * intensity, 0.8f * intensity, 0.1f * intensity, 1.0f }; // Amber (琥珀)
...
void MirrorShatterScript::GenerateGlassPanel(GameScene* scene, int count) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        sp.sizeX = sX; sp.sizeY = sY; sp.appearDelay = distRatio * 0.3f;
...
void MirrorShatterScript::Update(entt::entity entity, GameScene* scene, float dt) {
    ...
    for (auto& s : lightStreaks_) {
        if (timer_ > s.delay && s.life > 0) s.life -= dt;
    }
    // Sparks
    for (auto& sp : sparks_) {
        if (sp.life > 0) {
...
void MirrorShatterScript::Draw(entt::entity /*entity*/, GameScene* scene) {
    ...
    for (const auto& s : lightStreaks_) {
        if (timer_ < s.delay || s.life <= 0) continue;
