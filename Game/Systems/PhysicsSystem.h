...
public:
	void Update(entt::registry& registry, GameContext& ctx) override {
		...
		for (auto entity : dynamicView) {
			...
			if (!rb.enabled || rb.isKinematic || !bc.enabled) continue;
			...
			CollidableBox cb;
			cb.entity = entity;
			GetObbAxes(tc, bc, cb.axes, cb.center, cb.extents);
				...
				if (ctx.renderer->GetCollisionResult(req.resultIdx, ci)) {
					...
					if (registry.all_of<RigidbodyComponent>(req.entity)) {
						...
						if (dotVN < 0) {
							...
							// ★修正: 以前の 「vel *= 0.9f」 のような過剰な減衰を削除。
							// 接地判定の強化
							if (ci.normal.y > 0.8f) {
								// 衝突位置がキャラクターの下半分である場合のみ接地と判定
...
private:
	static void GetObbAxes(const TransformComponent& tc, const BoxColliderComponent& cb,
		Engine::Vector3 axes[3], Engine::Vector3& center, Engine::Vector3& extents) {
		Engine::Transform t;
		t.translate = {tc.translate.x, tc.translate.y, tc.translate.z};
		t.rotate = {tc.rotate.x, tc.rotate.y, tc.rotate.z};
		t.scale = {tc.scale.x, tc.scale.y, tc.scale.z};
		Engine::Matrix4x4 mat = t.ToMatrix();
		...
		extents.x = cb.size.x * 0.5f * std::abs(tc.scale.x);
		extents.y = cb.size.y * 0.5f * std::abs(tc.scale.y);
		extents.z = cb.size.z * 0.5f * std::abs(tc.scale.z);
	}
};
